...
func main() {
	...
	for _, intersection := range intersections {
		...
		dist := intersection.X + intersection.Y
		if dist < shortest  && dist > 0 {
			shortest = dist
		}
	}
	fmt.Println(redCoords)
	fmt.Printf("Manhattan distance: %d\n", shortest)
	// intersections := findIntersections(redVectors,redVectors)
	// fmt.Println("Intersections at:", intersections)
	// fmt.Println("Distance to Point:",  distanceToPoint([2]int{1,2},redVectors))
	shortest = math.MaxInt64
	for _, intersection := range intersections {
		redDist := distanceToPoint(intersection, redCoords)
		greenDist := distanceToPoint(intersection, greenCoords)
		dist := redDist + greenDist
		fmt.Printf("Distance detected: %d + %d = %d\n", redDist, greenDist, dist)
		if dist < shortest  && dist > 0 {
			shortest = dist
		}
	}
	fmt.Printf("Shortest Wire distance: %d\n", shortest)
}

func coordList (wireDetails string) []coordinate {
	coords := []coordinate{}
	X := 0
	Y :=  0
	coords = append(coords, coordinate{X:X, Y:Y})

	for _, direction := range strings.Split(wireDetails, ",") {
...
func findIntersections( redWire []coordinate, greenWire []coordinate) []coordinate {

	intersections := []coordinate{}
	for redPos := 0; redPos < len(redWire) -1 ; redPos++  {
		startXRed := redWire[redPos].X
		startYRed := redWire[redPos].Y
		endXRed := redWire[redPos+1].X
		endYRed := redWire[redPos+1].Y
		if startYRed > endYRed {
			startYRed, endYRed = endYRed, startYRed
		}
		if startXRed > endXRed {
			startXRed, endXRed = endXRed, startXRed
		}
		for greenPos := 0; greenPos < len(greenWire)-1 ; greenPos++ {
			startXGreen := greenWire[greenPos].X
			startYGreen := greenWire[greenPos].Y
			endXGreen := greenWire[greenPos+1].X
			endYGreen := greenWire[greenPos+1].Y
			if startYGreen > endYGreen {
				startYGreen, endYGreen = endYGreen, startYGreen
			}
			if startXGreen > endXGreen {
				startXGreen, endXGreen = endXGreen, startXGreen
			}
			if (startXGreen == endXGreen && startXRed != endXRed) || startXGreen != endXGreen && startXRed == endXRed  {	
				if startXGreen <= startXRed && startXRed <= endXGreen && startYRed <= startYGreen && startYGreen <= endYRed {
					intersections = append(intersections, coordinate{X:startXRed, Y: startYGreen })
				} else if startXRed <= startXGreen && startXGreen <= endXRed && startYGreen <= startYRed && startYRed <= endYGreen {
					
						intersections = append(intersections, coordinate{X: startXGreen, Y:startYRed})
				}
			}
		}
	}
...
func distanceToPoint(destination coordinate, path []coordinate) int {
	dist := 0
	type visitedSquare struct {
		count int
		loopDetector int
	}
	visited := make(map[coordinate]visitedSquare)
	loop := 0
	for pos := 0; pos < len(path) -1 ; pos++  {
		...
		for X != endX || Y != endY {
			...
			// fmt.Println("Wire at", X, ",", Y)

			if visited[coordinate{X:X, Y:Y}].count != 0  && visited[coordinate{X:X, Y:Y}].count < dist {
				dist = visited[coordinate{X:X, Y:Y}].count
				loop++
			} else if X == destination.X && Y == destination.Y {
				return dist
			}
			dist++
			visited[coordinate{X:X, Y:Y}] = visitedSquare{count: dist, loopDetector: loop}
